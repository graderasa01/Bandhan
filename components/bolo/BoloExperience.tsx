...
import { ArrowRight, FileUp, Keyboard, Loader2, Mic, PhoneOff, Sparkles, User, Users } from "lucide-react";
import {
  BOLO_DRAFT_KEY,
  MINIMUM_LIVE_KEYS,
  acceptAnswers,
...
export default function BoloExperience({ channels, voiceAvailable }: Props) {
  ...
  const [done, setDone] = useState<{ landing: string; live: boolean; existingAccount: boolean } | null>(null);
  ...
  const applyAnswers = useCallback((incoming: Record<string, unknown>) => {
    const result = acceptAnswers(draftRef.current.values, incoming);
    const next: BoloDraft = { ...draftRef.current, values: result.values, updatedAt: Date.now() };
  ...
  const setWho = useCallback((who: FillingFor) => {
    const next: BoloDraft = { ...draftRef.current, fillingFor: who, updatedAt: Date.now() };
    draftRef.current = next;
    setDraft(next);
  }, []);
  ...
  const completeAccount = useCallback(async () => {
    if (finishedRef.current) return { status: "live" as const };
    ...
    try {
      ...
      if (body.ok && body.landing) {
        ...
        landingRef.current = body.landing;
        try {
          localStorage.removeItem(BOLO_DRAFT_KEY);
        } catch {
          /* nothing to clear */
        }
        setDone({ landing: body.landing, live: Boolean(body.live), existingAccount: Boolean(body.existingAccount) });
        setStage("done");
        haptic("success");
        // Give Grio her one goodbye line, then go. The dashboard is the reward.
        redirectTimer.current = window.setTimeout(() => {
          router.replace(body.landing as string);
          router.refresh();
        }, sessionRef.current && sessionRef.current.currentStatus !== "closed" ? 6000 : 1200);
    ...
    async (call: ToolCallRequest): Promise<Record<string, unknown>> => {
      switch (call.name) {
        ...
        case "verify_otp": {
          ...
          return await verifyOtp(rawCode);
        }
        case "finish": {
          setStage("contact");
          const missingNow = missingMinimum(draftRef.current.values);
          if (missingNow.length > 0 && !contactRef.current.trim()) {
            return { status: "saved", missing: labelsFor(missingNow) };
          }
          return await completeAccount();
        }
        default:
          return { status: "unknown_tool" };
      }
    },
    [applyAnswers, completeAccount, sendOtp, setWho, verifyOtp],
  );
    ...
    (event: LiveEvent) => {
      switch (event.type) {
        ...
        case "turn_complete": {
          ...
          if (event.type === "turn_complete" && finishedRef.current && redirectTimer.current !== null) {
            // Grio has said goodbye — no need to wait the full grace period.
  ...
  const confirmReview = useCallback(() => {
    haptic("tap");
    setStage("contact");
    sessionRef.current?.sendText("[User ne review card par 'sahi hai' dabaya — ab contact poochho]");
  }, []);
  ...
  const uiVerify = useCallback(
    async (rawCode: string) => {
      ...
      if (result.status === "verified") {
        sessionRef.current?.sendText("[OTP verify ho gaya — ab finish call karo]");
        if (!sessionRef.current) await completeAccount();
      }
    },
    [completeAccount, verifyOtp],
  );

  const uiFinish = useCallback(async () => {
    setNotice(null);
    const result = await completeAccount();
    if (result.status === "live" || result.status === "saved") sessionRef.current?.sendText("[Profile ban gayi — ek line me badhai do aur alvida]");
  }, [completeAccount]);
    ...
    return (
      <section className="mx-auto max-w-md space-y-5 text-center">
        ...
        <Button
          ...
          fullWidth
          onClick={() => {
            router.replace(done.landing);
            router.refresh();
          }}
        >
          {t("bolo.done.continue", "Continue")}
