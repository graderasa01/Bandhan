...
const STATUS_LABEL: Record<MatchmakerRequestSummary["status"], { key: string; label: string }> = {
  OPEN: { key: "subscription.matchmakerOpen", label: "Request bhej di gayi" },
  CONTACTED: { key: "subscription.matchmakerContacted", label: "Team ne contact kiya" },
  RESOLVED: { key: "subscription.matchmakerResolved", label: "Poora hua" },
};
...
export default function MatchmakerRequestCard({ requests }: { requests: MatchmakerRequestSummary[] }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        toast({ title: t("subscription.matchmakerFailed", "Request nahi bheji ja saki"), description: json.message, tone: "error" });
        return;
      }
      toast({
        title: t("subscription.matchmakerSent", "Request bhej di"),
        description: t("subscription.matchmakerSentNote", "Hamari team jald aapse contact karegi."),
        tone: "success",
      });
      setNote("");
      router.refresh();
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[0.9375rem] font-semibold text-ink">
            {t("subscription.matchmakerTitle", "Talk to a Matchmaker")}
          </p>
          <p className="mt-0.5 text-[0.8125rem] leading-relaxed text-muted">
            {t(
              "subscription.matchmakerBlurb",
          ...
          <textarea
            ...
            placeholder={t("subscription.matchmakerPlaceholder", "Kis cheez me madad chahiye? (optional)")}
            rows={2}
          ...
          <Button variant="primary" size="sm" disabled={busy} onClick={submit} className="mt-2">
            {t("subscription.matchmakerCta", "Request a Call")}
          </Button>
              ...
              {requests.slice(0, 3).map((r) => (
                <li key={r.id} className="flex items-center justify-between text-[0.75rem] text-muted">
                  ...
                  <span className={r.status === "RESOLVED" ? "text-trust" : "text-gold-700"}>
                    {t(STATUS_LABEL[r.status].key, STATUS_LABEL[r.status].label)}
                  </span>
                </li>
              ))}
            </ul>
          ...
          {openCount >= 3 && (
            <p className="mt-2 text-[0.75rem] text-warn">
              {t("subscription.matchmakerLimit", "Aapki 3 requests already khuli hain.")}
            </p>
          )}
        </div>
      </div>
