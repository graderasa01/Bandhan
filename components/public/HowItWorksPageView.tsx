      ...
      <Container size="wide" className="flex flex-col gap-5 pb-16 pt-4 sm:gap-7 sm:pb-20 sm:pt-6">
        ...
        <section className="pc-shell px-6 py-12 sm:px-10 sm:py-14 lg:px-14">
          ...
          <RevealGroup className="relative mx-auto max-w-2xl">
            <div
              aria-hidden
              className="pc-thread pc-thread--v absolute bottom-8 left-[1.375rem] top-8"
            />

            {data.steps.map((step) => (
              <RevealItem key={step.step} className="relative flex gap-5 pb-8 last:pb-0">
                <span className="pc-step relative z-10 shrink-0">
                  {String(step.step).padStart(2, "0")}
                </span>
                <div className="pc-card min-w-0 flex-1 p-5">
                  <h2 className="pc-display text-[1.15rem] leading-snug">{step.title}</h2>
