 ...
 */
export default function SharedDeepProfileCard({ state }: { state: SharedDeepProfileCardState }) {
  if (state.state === "locked") {
    return (
      <Card variant="soft" padding="lg">
        <div className="flex items-center gap-2">
          <Lock className="size-4 shrink-0 text-subtle" />
          <p className="text-[0.9375rem] font-semibold text-ink">Deep Compatibility Report</p>
        </div>
        <p className="mt-1.5 text-[0.8125rem] leading-snug text-muted">
          Inhone apna Deep Profile ({state.computedCount} dimensions) aapke saath share kiya hai — dekhne ke
          liye Premium chahiye.
        </p>
        <Link href="/pricing">
  ...
  return (
    <Card variant="soft" padding="lg">
      ...
      <p className="mt-1.5 text-[0.8125rem] leading-snug text-muted">Inhone ye apni marzi se aapke saath share kiya hai.</p>
      <div className="mt-3">
