...
const DEMO_MESSAGES: Message[] = [
  { role: "ai", text: "Namaste! Main aapki profile complete karne me help kar sakta hu. Kya aap chahte hain ki main missing details identify karu?" },
  { role: "user", text: "Haan, meri missing details batao." },
  { role: "ai", text: "Abhi aapki height, current city aur family details missing hain. Sabse pehle aapki height kya hai?" },
];
...
export default function AIDrawer({
  ...
  return (<>
      ...
      <div ref={sheetRef} role="dialog" aria-modal="true" aria-label="AI Assistant panel"
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"var(--space-3) var(--space-4)",borderBottom:"1px solid var(--color-border)",flexShrink:0}}>
          <div><h3 style={{fontSize:"var(--text-base)",fontWeight:"var(--font-semibold)",color:"var(--color-text)",margin:0}}>AI Assistant</h3>
            <p style={{fontSize:"var(--text-xs)",color:"var(--color-text-muted)",margin:0}}>Current Page: {contextPage}</p></div>
          <button onClick={onClose} aria-label="Close AI drawer" style={{background:"none",border:"none",cursor:"pointer",color:"var(--color-text-muted)",padding:"var(--space-1)",borderRadius:"var(--radius-sm)",display:"flex",minWidth:"var(--touch-min)",minHeight:"var(--touch-min)",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{padding:"var(--space-3) var(--space-4)",borderBottom:"1px solid var(--color-border)",flexShrink:0}}>
          <p style={{fontSize:"var(--text-xs)",fontWeight:"var(--font-medium)",color:"var(--color-text-muted)",marginBottom:"var(--space-2)"}}>Suggested Actions</p>
        ...
        <div style={{padding:"var(--space-3) var(--space-4)",borderTop:"1px solid var(--color-border)",flexShrink:0,backgroundColor:"var(--color-surface)"}}>
          <div style={{display:"flex",gap:"var(--space-2)"}}>
            <input type="text" value={inputValue} onChange={e=>setInputValue(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();handleSend()}}} placeholder="Apna message likhein..." disabled={thinking} aria-label="AI chat input"
