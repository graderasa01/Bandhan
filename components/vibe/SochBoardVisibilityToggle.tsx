  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-line px-3.5 py-3">
      <div className="min-w-0">
        <p className="text-[0.875rem] font-medium text-ink">Meri Soch Board dikhe</p>
        <p className="text-[0.75rem] text-muted">Off karne par koi bhi aapki poll answers nahi dekh payega.</p>
      </div>
      <button
        type="button"
        role="switch"
        aria-checked={visible}
        aria-label="Soch Board visibility"
        disabled={busy}
        onClick={toggle}
        className={cn(
