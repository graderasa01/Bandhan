    ...
    <AdminShell adminName={user.fullName}>
      <div className="mx-auto max-w-4xl">
        <section className="mb-6">
          <h1 className="text-2xl font-bold text-wine-700">{t("verification.adminPage.title", "Photo Verification")}</h1>
          <p className="mt-2 text-sm text-muted">
            {pending.length > 0
              ? `${pending.length} ${t(
                  "verification.adminPage.pendingSuffix",
                  "photo review ke liye pending hain. Approve karte hi user ka trust score badhta hai aur uski profile par verified badge dikhne lagta hai.",
                )}`
              : t("verification.adminPage.allReviewed", "Sab photos review ho chuki hain.")}
          </p>
        </section>
