...
/**
 * "Upload Photo" on /admin/theme. Admin only, one file. Bounded by
 * `content-length` before the body is read and by size again after; the MIME
 * type is a courtesy check only — what the file decodes as is decided in the
 * service, and what is stored is a WebP the service wrote.
 *
 * Nothing goes live here. The response is the processed photo (URLs, the
 * measured brightness and the dim it needs), for the phone preview; the room
...
export async function POST(req: Request) {
  ...
  try {
    const photo = await uploadRoomPhoto({ buffer: Buffer.from(await file.arrayBuffer()), actorId: user.id });
