...
export default async function InterestsPage() {
  ...
  const data = await getInterestsData(user.id, t);

  return (
    <UserShell userName={user.fullName}>
      <div className="mx-auto max-w-3xl">
        <section className="mb-6">
          <h1 className="text-2xl font-bold text-accent-text">{t("userPages.interests.title", "Interests")}</h1>
          <p className="mt-2 text-sm text-muted">
            {t("userPages.interests.subtitle", "Received aur sent interests ka overview.")}
          </p>
        </section>
